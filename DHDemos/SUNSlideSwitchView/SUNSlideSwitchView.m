...
{
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMinY(self.bounds), [UIScreen mainScreen].bounds.size.width, kHeightOfTopScrollView)];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_topScrollView.frame), [[UIScreen mainScreen] bounds].size.width, 1)];
    [line setBackgroundColor:kUIColorFromRGB(0xebe8ed)];
//    [line setAlpha:0.4];
    [self addSubview:line];
    
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(line.frame)+10, self.bounds.size.width, self.bounds.size.height - kHeightOfTopScrollView)];
...
//                                   lineBreakMode:NSLineBreakByTruncatingTail];
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 49, textSize.width, _shadowImage.size.height);
            button.selected = YES;
        }
...
- (void)createNameButtons_new
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,CGRectGetMinY(_topScrollView.bounds),width, kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        xOffset += width + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 49+64, width, _shadowImage.size.height);
            button.selected = YES;
        }
        [button setTitle:vc.title forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont systemFontOfSize:kFontSizeOfTabButton];
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x, 49+64, sender.frame.size.width, _shadowImage.size.height)];
